document.addEventListener('DOMContentLoaded', () => {
    ...
    function analyzeHTML(htmlContent) {
        ...
        // Inject IDs into editable elements
        const elements = processEditableElements(doc);

        // Serialize back to string for preview
        const taggedHTML = doc.documentElement.outerHTML;

        // Generate a pseudo-token (just for internal consistency or remove)
        const token = 'local-session';
        // We don't need a token really, just store it
        window.CURRENT_HTML = taggedHTML;

        initializeEditor(taggedHTML, elements);
    }

    function processEditableElements(doc) {
        const editableElements = [];
        const targetTags = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'a', 'img', 'span', 'li', 'button'];

        // Helper to generate IDs
        const generateId = () => "ai-edit-" + Math.random().toString(36).substr(2, 8);

        doc.querySelectorAll(targetTags.join(',')).forEach(tag => {
            const hasText = tag.textContent.trim().length > 0;
            const isMedia = tag.tagName.toLowerCase() === 'img';
            const isLink = tag.tagName.toLowerCase() === 'a';

            if (!hasText && !isMedia) return;

            let uid = tag.getAttribute('data-ai-id');
            if (!uid) {
                uid = generateId();
                tag.setAttribute('data-ai-id', uid);
            }

            const elementData = {
                id: uid,
                tag: tag.tagName.toLowerCase(),
                label: hasText ? `${tag.tagName} - ${tag.textContent.trim().substring(0, 30)}...` : tag.tagName,
                content: tag.textContent.trim(),
                attributes: {}
            };

            if (isMedia) {
                elementData.attributes['src'] = tag.getAttribute('src') || '';
                elementData.label += " (Image)";
            }
            if (isLink) {
                elementData.attributes['href'] = tag.getAttribute('href') || '';
            }

            editableElements.push(elementData);
        });

        return editableElements;
    }

    function initializeEditor(taggedHTML, elements) {
        uploadView.classList.add('hidden');
        editorView.style.display = 'flex';

        // Use srcdoc for immediate rendering without server
        previewFrame.srcdoc = taggedHTML;
